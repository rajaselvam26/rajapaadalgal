...
i. class ==> can be create any name 
...
i. SESSION POP UP SET ON USER PANEL ===> EXPIRY TIME


STEPS TO FOREIGN KEY:

i. while create the parent table using Innodb

ii.If you set the innodb ==> relation view you can see

iii. CASCADE for update and delete

ALTER TABLE `cat_product` ADD FOREIGN KEY (`cat_id`) REFERENCES 

`dev__frame`.`category`(`cat_id`) ON DELETE CASCADE ON UPDATE CASCADE;
